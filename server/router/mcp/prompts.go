...
func (s *MCPService) registerPrompts(mcpSrv *mcpserver.MCPServer) {
	...
	// daily_log — guides structured daily-log creation via save_daily_log.
	mcpSrv.AddPrompt(
...
func (*MCPService) handleDailyLogPrompt(_ context.Context, req mcp.GetPromptRequest) (*mcp.GetPromptResult, error) {
	content := req.Params.Arguments["content"]
	if content == "" {
		return nil, errors.New("content argument is required")
	}
	...
	return &mcp.GetPromptResult{
		Description: "Record a daily log",
		Messages: []mcp.PromptMessage{
			mcp.NewPromptMessage(mcp.RoleUser, mcp.NewTextContent(instruction)),
		},
	}, nil

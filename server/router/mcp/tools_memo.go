...
func (s *MCPService) registerMemoTools(mcpSrv *mcpserver.MCPServer) {
	...
	mcpSrv.AddTool(mcp.NewTool("memos_get_memo",
	...
	mcpSrv.AddTool(mcp.NewTool("memos_update_memo",
	...
	mcpSrv.AddTool(mcp.NewTool("memos_delete_memo",
	...
	mcpSrv.AddTool(mcp.NewTool("memos_search_memos",
	...
	mcpSrv.AddTool(mcp.NewTool("memos_list_memo_comments",
	...
	mcpSrv.AddTool(mcp.NewTool("memos_create_memo_comment",
...
func (s *MCPService) handleListMemos(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to list memos: %v. Check your filter syntax or try without filter.", err)), nil
	}
	...
	out, err := marshalJSON(listResponse{Memos: results, HasMore: hasMore, Page: page, PageSize: pageSize})
	if err != nil {
		return nil, err
	}
...
func (s *MCPService) handleCreateMemo(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if content == "" {
		return mcp.NewToolResultError("content is required. Provide memo content in Markdown format."), nil
	}
...
func (s *MCPService) handleUpdateMemo(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if memo == nil {
		return mcp.NewToolResultError(fmt.Sprintf("memo not found: %s. Use memos_list_memos to see your memos.", uid)), nil
	}
	if memo.CreatorID != userID {
		return mcp.NewToolResultError("permission denied. You can only update memos you created."), nil
	}
...
func (s *MCPService) handleDeleteMemo(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if memo.CreatorID != userID {
		return mcp.NewToolResultError("permission denied. You can only delete memos you created."), nil
	}
...
func (s *MCPService) handleSearchMemos(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if query == "" {
		return mcp.NewToolResultError("query is required. Provide text to search for in memo content."), nil
	}
...
func (s *MCPService) handleCreateMemoComment(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	content := req.GetString("content", "")
	if content == "" {
		return mcp.NewToolResultError("content is required. Provide comment content in Markdown format."), nil
	}

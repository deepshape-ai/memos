...
type dailyLogJSON struct {
	...
	Creator    string `json:"creator"`     // "users/{id}"
	Content    string `json:"content"`     // .plan-style prefixed content
	Date       string `json:"date"`        // YYYY-MM-DD
	Editable   bool   `json:"editable"`    // whether caller can still modify
...
func (s *MCPService) registerDailyLogTools(mcpSrv *mcpserver.MCPServer) {
	mcpSrv.AddTool(mcp.NewTool("save_daily_log",
	...
	mcpSrv.AddTool(mcp.NewTool("get_daily_log",
	...
	mcpSrv.AddTool(mcp.NewTool("list_daily_logs",
...
func (s *MCPService) handleSaveDailyLog(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if existing == nil {
		...
		if !isDailyLogToday(existing) {
			return mcp.NewToolResultError("past daily logs cannot be modified; use +/- prefixes in today's log instead"), nil
		}
		visibility := store.Protected
...
func (s *MCPService) handleGetDailyLog(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if memo == nil {
		return mcp.NewToolResultError(fmt.Sprintf("no daily log found for %s", dateStr)), nil
	}
...
func (s *MCPService) handleListDailyLogs(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if raw := req.GetString("start_date", ""); raw != "" {
		...
		if parseErr != nil {
			return mcp.NewToolResultError("invalid start_date, expected YYYY-MM-DD"), nil
		}
		ts := t.Unix()
		startTs = &ts
	}
	if raw := req.GetString("end_date", ""); raw != "" {
		t, parseErr := time.Parse("2006-01-02", raw)
		if parseErr != nil {
			return mcp.NewToolResultError("invalid end_date, expected YYYY-MM-DD"), nil
		}
		ts := t.Unix()
		endTs = &ts
	}
	...
	out, err := marshalJSON(listResponse{DailyLogs: results, HasMore: hasMore})
	if err != nil {
		return nil, err
	}
	return mcp.NewToolResultText(out), nil
